  ...
  getMirrors(function(mirrorsT) {
    ...
    for (var i = 0; i < mirrors.length; i++) {
      ...
      if (typeof betaversion != 'undefined') {
        chrome.browserAction.setTitle({title: "All Mangas Reader " + betaversion});
      } else {
        chrome.browserAction.setTitle({title: "All Mangas Reader " + curVersion});
      }
...
chrome.extension.onRequest.addListener(
  function(request, sender, sendResponse) {
    ...
    if (request.action == "mangaInfos") {
      var mangaExist = isInMangaList(request.url);
      if (mangaExist != null) {
        sendResponse({read: mangaExist.read, display: mangaExist.display});
      } else {
        sendResponse(null);
      }
    }
    ...
    if (request.action == "openExtensionMainPage") {
      //TO_CHANGE
      chrome.tabs.create({"url": "http://www.allmangasreader.com/"});
      try {
    ...
    if (request.action == "actmirrors") {
      getActivatedMirrorsWithList({list: activatedMirrors()}, function(mirrorsDesc) {
    ...
    if (request.action == "nbMangaInMirror") {
      ...
      sendResponse({number: nb, mirror: request.mirror});
    }
    ...
    if (request.action == "isMirrorActivated") {
      sendResponse({mirror: request.mirror, activated: isMirrorActivated(request.mirror)});
    }
    if (request.action == "activatedMirrors") {
      var lst = activatedMirrors();
      sendResponse({list: lst});
    }
    ...
    if (request.action == "getBookmarkNote") {
      ...
      sendResponse({isBooked: noteBM.booked, note: noteBM.note, scanSrc: noteBM.scanSrc});
    }
      ...
      if (!isFound) {
        ...
        var id = chrome.contextMenus.create({
          ...
          onclick: function(info, tab) {
            chrome.tabs.executeScript(tab.id, {code: "clickOnBM(\"" + info.srcUrl + "\")"}, function() {}); 
          },
    ...
    if (request.action == "importMangas") {
      sendResponse({out: importMangas(request.mangas, request.merge)});
    }
    if (request.action == "importBookmarks") {
      sendResponse({out: importBookmarks(request.bookmarks, request.merge)});
    }
    if (request.action == "hideBar") {
      ...
      sendResponse({res: localStorage["isBarVisible"]});
    }
    ...
    if (request.action == "barState") {
      if (localStorage["isBarVisible"] == undefined) {
        sendResponse({barVis: 1});
      } else {
        sendResponse({barVis: localStorage["isBarVisible"]});
      }
    }
    ...
    if (request.action == "importimplementation") {
      importImplentationFromId(request.id, function(mirrorName) {
        updateMirrors(function() {
          sendResponse({mirror: mirrorName}); 
        });
      });
    }
    ...
    for (var j = 0; j < bookmarks.length; j++) {
      if (obj.mirror == bookmarks[j].mirror 
        ...
        if (obj.type == "chapter") {
          ...
          return {booked: true, note: bookmarks[j].note};
        } else {
          if (obj.scanUrl == bookmarks[j].scanUrl || encodeURI(obj.scanUrl) == bookmarks[j].scanUrl) {
            ...
            return {booked: true, note: bookmarks[j].note, scanSrc: obj.scanUrl};
          }
        }
      }
    }
  }
  return {booked: false, note: ""};
}
  ...
  if (lstTmp.length > 0) {
    for (var j = 0; j < lstTmp.length; j++) {
      if (lstTmp[j].mirror == mirrorName) {
        lstTmp[j] = {mirror: mirrorName, activated: true};
...
function desactivateMirror(mirrorName) {
  ...
  if (nb == 0) {
    ...
    if (lstTmp.length > 0) {
      for (var j = 0; j < lstTmp.length; j++) {
        if (lstTmp[j].mirror == mirrorName) {
          lstTmp[j] = {mirror: mirrorName, activated: false};
          localStorage["mirrorStates"] = $A(lstTmp).toJSON();
    ...
    if (lstTmp.length > 0) {
      ...
      for (var i = 0; i < mirrors.length; i++) {
        ...
        if (!isFound) {
          ...
          lstTmp[lstTmp.length] = {mirror: mirrors[i].mirrorName, activated: true};
          toUpdate = true;
        }
      }
      if (toUpdate) {
...
function instantiateMirrors() {
  var lst = new Array();
  for (var i = 0; i < mirrors.length; i++) {
    lst[lst.length] = {mirror: mirrors[i].mirrorName, activated: true};
  }
...
function refreshManga(mg, waiter, pos) {
  if (getMangaMirror(mg.mirror) == null) return;
...
function refreshAllLasts(refreshTimer, perform) {
  try {
    if (perform == undefined || perform == true) {
      ...
      var nbToRef = 0;
      for (var i = 0; i < mangaList.length; i++) {
        if (getMangaMirror(mangaList[i].mirror) != null) nbToRef++
      }
  ...
  if (nbNews == 0) {
    ...
    if (params.nocount == 1) {
      ...
      chrome.browserAction.setBadgeText({text: ""});  
    	...
    	chrome.browserAction.setBadgeText({text: nbNews.toString()});  
